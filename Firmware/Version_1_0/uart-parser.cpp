...
void uart_parser(void) {
  if (uart2->available()) {
    uart2->readStringUntil('\n').toCharArray(uart_rx_buffer, UART_BUFFER_SIZE);
      ...
      if((sample_rate_4k && toggle)  || !sample_rate_4k) {
        ...
        if (wifi_enable) {
          if (wifi_connected) {
            ...
            debug_counter++;
            udp.write((uint8_t *) &data.adc[0], 4*7);
            udp.write('\r');
            udp.write('\n');
          ...
          for(uint8_t i = 0; i < 6; i ++) {
            buffer = (uint8_t *) &data.adc[i];
            memcpy(&sendBuf[inPtr], (++buffer), 3);
            inPtr += 3;
          }
          uart2->write((uint8_t*)&sendBuf[0], 20);
          ...
          //uart2->write((uint8_t *) data.adc, 4*6);  // 6 ADCs
